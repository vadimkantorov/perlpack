...
for p in files:
    ...
    print('{ "' + repr(pp)[1:-1] + '", "' + repr(os.path.join(args.prefix, p))[1:-1] + '", ', f'_binary_{pp}_start, ', f'_binary_{pp}_end ', '},', file=f)
print('};', file=f)
print('const char* packfsdirs[] = {\n', ',\n'.join('"' + repr(os.path.join(args.prefix, p))[1:-1] + '"' for p in dirs), '\n};\n', file=f)
